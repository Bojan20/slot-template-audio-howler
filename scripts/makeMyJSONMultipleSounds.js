...
const files = { process: process.argv[3] };
console.log("audioProcess type - ", audioProcess.process + " soundFiles - " + files.process);
const fs = require("fs");
...
if (audioProcess.process === "audioSprite") {
    let sndFilesData;
    if(files.process !== undefined && files.process > 0) {
        sndFilesData = [];
        sndDataEntries = [];
        sndSpriteEntries = [];
        for(let i=0; i<files.process; i++) {
        ...
        if (element.endsWith("_SL.wav")) {
            ...
            if (audioProcess.process === "audio") {
                ...
                if(files.process > 0 ) {
                    ...
                    let srcPath;
                    if (Object(sndSpriteEntries[increment]).hasOwnProperty(spriteId) === true) {
                        srcPath = sndDataEntries[increment].get("src")[0];
                    } else if (Object(sndSpriteEntries[increment + 1]).hasOwnProperty(spriteId) === true) {
                        increment++;
                        srcPath = sndDataEntries[increment].get("src")[0];
                    } else if(Object(sndSpriteEntries[increment + 2]).hasOwnProperty(spriteId) === true) {
            ...
            sox.identify(SourceSoundDirectory + '/' + element, function(err, results) {
                ...
                */
                ...
                //console.log(" duration - " + duration + " Increment = " + increment1);
                let increment1 = 0;
                myNewEntry.soundId = soundId;                
                if (audioProcess.process === "audio") {
                    ...
                    if(files.process > 0) {
                        ...
                        if (Object(sndSpriteEntries[increment1]).hasOwnProperty(spriteId) === true) {
                            console.log(" Start Time - " + sndSpriteEntries[increment1][spriteId][0] + " Sprite " + spriteId);
                            myNewEntry.startTime = sndSpriteEntries[increment1][spriteId][0];
                        } else if (Object(sndSpriteEntries[increment1]).hasOwnProperty(spriteId) === false) {
                            increment1++;
                            if(Object(sndSpriteEntries[increment1]).hasOwnProperty(spriteId) === true) {
                                myNewEntry.startTime = sndSpriteEntries[increment1][spriteId][0];
                            } else if(Object(sndSpriteEntries[increment1]).hasOwnProperty(spriteId) === false) {
